...
The project uses the latest live data, token supply and price are pulled from CoinGecko while TVL and revenue come from DefiLlama. 
This data is cached to avoid feth]ching the same thing many times as the APIs have limited use. 
Then is calculates these five key indicators: TVL growth, future dilution potential, emission pressure, P/S ratio and FDV/revenue. 
And is combines them into the Composite score. Added a weight that changes based on whether the protocol's revenue flows to token holdersor not.
